            ...
            <v-tabs-window-item :value="1">
              <v-card-text>
                <v-form @submit.prevent="submitForm">
                  ...
                  <v-btn class="btn-group" type="submit" block>Register</v-btn>
                </v-form>
              </v-card-text>
            </v-tabs-window-item>
        ...
        <v-card outlined class="pa-4 mb-6">
          <v-card-title class="text-center">QR Code Generator</v-card-title>
           ...
           <v-radio-group v-model="useCurrentTime" inline color="#135389">
            ...
            <div>
                <p>ストアID: {{ selectedItems.storeId }}</p>
                <p>購入日時: {{ selectedItems.purchaseDate }}</p>
                <div v-for="item in selectedItems.items" :key="item.id">
                <p>商品ID: {{ item.id }}  量: {{ item.quantity }}</p>
                </div>
            </div>
            </v-alert>
          </v-card-text>
    ...
    async generateQRCode() {
      ...
      if (this.itemCount < 1 || this.minQuantity < 1 || this.maxQuantity < this.minQuantity) {
        ...
        for (let i = 0; i < this.itemCount; i++) {
            ...
            const selectedItem = availableItems.splice(randomIndex, 1)[0]; // ランダム選択と除去
            const randomQuantity = Math.floor(Math.random() * (this.maxQuantity - this.minQuantity + 1)) + this.minQuantity;
            selectedItems.push({ id: selectedItem.id, quantity: randomQuantity });
        }
